...
def _quotes(rows):
    return pd.DataFrame(
        [
            {"event_ts": timestamp, "bid": bid, "ask": ask}
...
def test_triggered_order_defaults_to_submitted_limit_not_better_ask() -> None:
    result = simulate_long_option_entry(
    ...
    assert result["limit_source"] == "candidate_explicit"
    assert "explicit_limit_exceeds_auto_chase_policy" in result["warnings"]
...
def test_less_conservative_mode_can_fill_at_trigger_ask_only_when_explicit() -> None:
    result = simulate_long_option_entry(
        _candidate(execution_limit_price=2.10),
        _quotes([("2026-08-24T15:00:02+00:00", 1.85, 1.90)]),
