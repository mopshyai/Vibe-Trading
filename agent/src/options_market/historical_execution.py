...
A historical candidate is treated as a buy-limit order submitted at the decision
time. After configured latency, later quote observations are inspected until the
wait window expires. The order fills only when an eligible ask is at or below the
submitted limit.

Without order-book queue/size history, fill certainty cannot be reconstructed.
For conservatism a triggered buy fills at the submitted limit price by default,
not a midpoint or better displayed ask. Results are evaluation labels only.
"""
...
def simulate_long_option_entry(
    ...
    *,
    ...
    auto_limit = decision_ask * (1.0 + cfg.max_chase_pct / 100.0)
    limit_price = explicit_limit or auto_limit
    limit_source = "candidate_explicit" if explicit_limit is not None else "decision_ask_plus_auto_chase"
    ...
    warnings: list[str] = []
    if explicit_limit is not None and explicit_limit > auto_limit + 1e-12:
        warnings.append("explicit_limit_exceeds_auto_chase_policy")
    if resolution is not None and resolution > cfg.max_wait_seconds:
        warnings.append("quote_resolution_coarse_for_wait_window")
    if (
        "ask_size" not in frame.columns
        or pd.to_numeric(frame.get("ask_size"), errors="coerce").dropna().empty
    ):
    ...
    eligible = frame[(frame["event_ts"] >= eligible_at) & (frame["event_ts"] <= expires_at)].copy()
    if eligible.empty:
        return {
            **_base(contract, decision_time, decision_ask, decision_bid, cfg),
            "status": "DATA_UNAVAILABLE",
