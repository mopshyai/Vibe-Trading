"""Apply the historical limit-order model to experiment selections.

Entry-path quote queries are capped by the order timeout both in event time and
`available_at`, preventing later data corrections from retroactively creating a
fill. Outcome paths are evaluation-only and may use the experiment's later
`created_as_of` timestamp.
"""
...
def apply_execution_model_to_experiment(
    ...
    *,
    ...
    """Re-evaluate selected candidates through a historical limit-order path."""
    ...
    experiment_config = experiment.get("experiment_config") if isinstance(experiment.get("experiment_config"), Mapping) else {}
    horizon_days = _positive_int(experiment_config.get("outcome_horizon_days")) or 30
    target_profit_pct = _positive(_mapping(experiment.get("replay_config")).get("target_profit_pct")) or 300.0
    ...
    original_outcomes = {
        ...
        if isinstance(row, Mapping) and _candidate_key(row) is not None
    }
    ...
    for raw in selected:
        ...
        if not contract or decision_time is None:
            ...
            results.append({"candidate": candidate, "execution": execution, "outcome": None})
            continue

        expires_at = decision_time + timedelta(seconds=cfg.max_wait_seconds)
        # Point-in-time execution path: no quote revision received after the
        # order timeout can affect whether this simulated order filled.
    ...
    execution_rows = [row["execution"] for row in results]
    filled_outcomes = [row["outcome"] for row in results if isinstance(row.get("outcome"), Mapping)]
    return {
        ...
        "warning": (
            "Quote-trigger execution model only. Without queue position, quote sizes, trades and full order-book history, a triggered limit is not proof that the real order would have filled."
        ),
    }
...
def _outcome_summary(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    n = len(rows)
    end_returns = [value for row in rows if (value := _finite(row.get("end_return_pct"))) is not None]
    return {
        "filled_outcomes": n,
