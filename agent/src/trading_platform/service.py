...
def _journal_entry(raw: Mapping[str, Any], *, snapshot: TradingDeskSnapshot) -> JournalEntry | None:
    ...
    return JournalEntry(
        ...
        surface_efficiency_score=_bounded100(raw.get("surface_efficiency_score")),
        surface_required_move_ratio=_nonnegative(raw.get("surface_required_move_ratio")),
        surface_iv_percentile=_bounded100(raw.get("surface_iv_percentile")),
        candidate_iv_premium_to_atm_points=_finite(raw.get("candidate_iv_premium_to_atm_points")),
        surface_atm_expected_move_pct=_nonnegative(raw.get("surface_atm_expected_move_pct")),
        surface_term_structure_state=_text(raw.get("surface_term_structure_state")),
        surface_skew_state=_text(raw.get("surface_skew_state")),
        surface_implied_vs_realized_state=_text(raw.get("surface_implied_vs_realized_state")),
